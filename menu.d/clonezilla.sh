...
while read -r folder; do
	...
	grub_linux_entry \
		...
		"${GRUB_PATH}/${kernels[0]}" \
		"${GRUB_PATH}/${initrds[0]}" \
		"boot=live netboot=nfs nfsroot=${PXE_NFS_HOST}:${NFS_PATH}/${folder} union=overlay username=user hostname=clonezilla config quiet components noswap edd=on nomodeset noeject ocs_live_run=ocs-live-general ocs_live_extra_param= ocs_live_batch=no vga=788 net.ifnames=0 splash i915.blacklist=yes radeonhd.blacklist=yes nouveau.blacklist=yes vmwgfx.enable_fbdev=1"
